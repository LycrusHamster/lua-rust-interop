...
fn main() -> Result<()> {
    ...
    lua.context(|lua_ctx| {
        ...
        //=============
        //传给lua
        let v:Vec<u8> = vec![0x48_u8,0x65_u8,0x6c_u8,0x6c_u8,0x65_u8];
        globals.set("u8_bytes", v)?;
    ...
    lua.context(|lua_ctx| {
        ...
        //=============
        //从lua中读取
        let v = globals.get::<_,Vec<u8>>("u8_bytes")?;
        println!("get u8_bytes : {:?}", v);
    ...
    lua.context(|lua_ctx| {
        ...
        lua_ctx
            .load(
                r#"
                  ...
                  for k, v in pairs(vec_to_lua) do
                    print(k, v)
                  end
            "#,
            )
        ...
        let non_utf8: rlua::Value = lua_ctx.load(r#"  "test\xff"  "#).eval()?;
        globals.set("non_utf8_to_lua", non_utf8)?;
        let non_utf8 = globals.get::<_, rlua::Value>("non_utf8_to_lua")?;
        println!("get non_utf8_to_lua : {:?}", non_utf8);
        ...
        lua_ctx
            .load(
                r#"
                 string_to_rust = "this is string for rust"
            "#,
            )
            .exec()?;

        let v = globals.get::<_,String>("string_to_rust")?;
        println!("string_to_rust : {:?}", v);


        let non_utf8: rlua::Value = lua_ctx.load(r#"  "test\xff"  "#).eval()?;

        if let rlua::Value::String(s) = non_utf8 {
            //let ss = s.to_str().expect("goes wrong");
            //println!("non_utf8 : {}", ss);

            let s = s.as_bytes();
            println!("non_utf8_to_rust : {:?}", s);
        }
